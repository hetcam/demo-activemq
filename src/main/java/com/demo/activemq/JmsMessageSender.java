	...
	public void send(final Destination dest, final String text) {
		...
		this.jmsTemplate.send(dest, new MessageCreator() {
			...
			public Message createMessage(Session session) throws JMSException {
				...
				Message message = session.createTextMessage(tokens[0]);
				message.setJMSMessageID(tokens[1]);
				message.setJMSTimestamp(Long.parseLong(tokens[2]));
				message.setJMSCorrelationID(tokens[3]);
				//message.setObjectProperty("clientID", null);
				return message;
			}
		});
	}
